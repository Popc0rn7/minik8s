...
如果从 `two-node.md` 连续执行到本文档，通常已经有 node-a/node-b 的 CNI 和 sailer。`POD-01` 会临时停掉 node-a 当前 sailer，并以禁用 CNI 的方式重启 node-a sailer；`POD-02` 到 `POD-04` 再恢复启用 CNI 的 sailer。
