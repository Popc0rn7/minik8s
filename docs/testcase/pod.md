# Pod Kubeharbor / Kubesailer 测试用例与实现映射
...
`MINIK8S_KUBEHARBOR` 让 CLI 通过 kubebridge 的 HTTP API 执行 `apply/get/delete`；未设置时这些命令会直接报错，避免绕过 kubebridge 读写状态。`MINIK8S_STATE_DIR` 是 kubebridge 的本地持久化目录，普通 CLI 客户端不直接读写该目录。`MINIK8S_PLAIN=1` 让 CLI 输出退回 ASCII，方便在报告和自动化断言中匹配 `DONE`、`WARN`、`[ok]` 等文本。Pod 基础 case 关注生命周期、端口、volume 和资源映射，因此设置 `MINIK8S_CNI_DISABLED=1`，避免已有 CNI 配置影响 hostPort 行为。`apply` 只向控制面提交期望状态，真正创建/重启/删除容器由独立 kubesailer 完成。kubesailer 需要 `sudo` 时要用 `sudo env ...` 传递输出和 CNI 相关环境变量。kubebridge 使用 `:18080` 是为了避开 POD-01 中 nginx 的 `hostPort: 8080`。
...
| 启动和终止 Pod | POD-01、POD-04 | 多个 manifest | `apply`、`kubesailer`、`delete pod` | 已实现，可验证 |
...
- `apply` 输出包含 `pod/volume-resource-pod created (Pending)`；kubesailer 同步后进入 `Running`。
...
- kubesailer 的轮询或 `kubesailer --once` 会触发一次 node-local sync。
- 再次 `docker inspect` 时同一个容器回到 `running`。
- `get pods` 输出仍显示 Pod 为 `Running`。

说明：当前实现通过 `internal/kubebridge/kubecaptain/pod_kubecaptain.go` 的 `handleRunningPod` 检查 runtime 状态，并在 `shouldRestart` 为 true 时调用 `StartContainer`。该逻辑已由独立 `minik8s kubesailer` 调用，CLI `apply/get/delete` 不再直接操作 Docker runtime。
...
目标：验证当 runtime 无法创建 sandbox 或容器时，kubesailer/PodKubecaptain 能写入 `Failed` 状态和失败原因。该 case 使用单元测试复现，因为真实 Docker 失败不稳定。

流程：

```bash
go test ./internal/kubebridge/kubecaptain ./internal/kubesailer -run 'SandboxCreationFailure|SyncOnce' -v
```

期望：

- kubecaptain 测试断言 Pod 进入 `Failed`。
- kubesailer 测试断言只处理分配给本节点的 Pod，并把 status 回写给 Kubeharbor。

关联代码：`internal/kubebridge/kubecaptain/pod_kubecaptain_test.go` 通过 mock runtime 稳定模拟 sandbox 创建失败；`internal/kubesailer/kubesailer_test.go` 覆盖独立 kubesailer 同步。

## 6. 实现设计映射

进程入口：`cmd/minik8s/main.go` 根据子命令延迟初始化 Docker runtime。kubebridge 控制面位于 `internal/kubebridge/`：`Kubebridge` 是对外暴露的控制面内核，Kubeharbor、file-backed store、kubecaptain 和 kubenavigator 都是其内部组件。`kubesailer` 才初始化 Docker runtime 和 CNI。
...
生命周期管理：`internal/cli/cli.go` 的 `apply` 通过 `MINIK8S_KUBEHARBOR` 向 kubebridge 提交 Pod，kubebridge 将初始状态写为 `Pending`；`internal/kubesailer` 通过 kubebridge API 拉取 `spec.nodeName` 等于本节点的 Pod，复用 `PodKubecaptain` 创建/重启/删除容器，并通过 status API 回写状态。
...
资源、端口、volume 映射：`internal/kubebridge/kubecaptain/pod_kubecaptain.go` 将 Pod spec 转成 runtime config；Docker runtime 将 ports 绑定到 sandbox，将 mounts 和 CPU/memory limits 应用到 workload 容器。

状态展示：`internal/cli/cli.go` 的 `get pods` 输出 `POD`、`STATUS`、`IP`、`UPTIME`、`NAMESPACE`、`LABELS`。`formatUptime` 只在 kubesailer 回写 `Running` 且有 `StartTime` 时显示运行时长。
