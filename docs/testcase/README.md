...
| --- | --- | --- | --- | --- |
| 控制面 + worker | `node-a` | `kubebridge`、`net-registry`、`netd`、`kubesailer` | `10.244.0.0/24` | API server 与 etcd 推荐放这里 |
| worker | `node-b` | `netd`、`kubesailer` | `10.244.1.0/24` | 只访问 Kubeharbor，不直连 etcd |
...
两台机器都需要 Linux、Docker、`ip`、`iptables`、`nsenter`、`curl` 或 `wget`，并能使用 `sudo`。`10.244.0.0/16` 不应与局域网或宿主机路由冲突。
...
如需完全重置测试状态，停止 `kubebridge`、`kubesailer`、`netd` 后再删除 `.minik8s/testcase-state`、`.minik8s/state/cni-ipam.json`，并清理残留 Docker 容器。
...
- 跨节点 PodIP 不通：检查 `netd` 是否运行，`ip route` 是否有对端 PodCIDR，宿主机防火墙是否拦截转发。
- Service ClusterIP 不通：检查 `kubebridge` 是否启用了默认 ServiceProxy，`sudo iptables-save -t nat | grep MK8S-SVC` 是否有规则。
