...
本目录记录 Minik8s v0.1.0 的验收 case。v0.1.0 的稳定边界是：Pod 生命周期、双 worker 心跳与调度、单/双节点 CNI、Service endpoints、iptables ClusterIP/NodePort、ReplicaSet 副本管理、etcd Pod/Service/ReplicaSet 持久化。不覆盖 HPA、DNS、Serverless、SecurityContext、GPU。
...
| CASE-04 | `replicaset.md` | ReplicaSet 创建、补副本、缩容、级联删除 | 是 |
| CASE-05 | `logbook.md` | Pod/Service/ReplicaSet 写入 etcd，bridge 重启后恢复 | 是 |
...
# 5. 在 node-a 执行 replicaset.md。
# 6. 需要持久化验收时执行 logbook.md。
```
...
如需完全重置测试状态，停止 `bridge`、`sailer` 后再删除 `.minik8s/testcase-state`、`.minik8s/state/cni-ipam.json`，并清理残留 Docker 容器。

## 常见故障定位

- `get nodes` 只有一个节点：检查 node-b 的 `HARBOR` 是否指向 node-a 局域网 IP，而不是 `127.0.0.1`。
- Pod 一直 `Pending`：执行 `./minik8s get nodes` 看是否有 Ready 节点；Pod 不应在 YAML 中指定 `nodeName`，分配由 scheduler 写入。
...
- ReplicaSet current 小于 desired：确认 bridge 的 `--replicaset-sync-interval` 未被设为 `0`，或执行 `./minik8s get rs` 触发一次同步。
