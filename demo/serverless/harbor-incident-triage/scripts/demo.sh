...
echo "[3] Functions before invoke; fn-* ReplicaSets should start at 0"
...
echo "[8] Publish an event to show EventTrigger -> Function"
"$MINIK8S" publish minik8s.incident.created --data "$(cat "$DEMO_ROOT/inputs/low-risk-incident.json")" || true
"$CLI" describe function normalize-input || true

echo "[9] Function pods after cold start"
"$CLI" get pods || true
"$CLI" get replicasets || true

echo "[10] Wait for scale-to-0"
